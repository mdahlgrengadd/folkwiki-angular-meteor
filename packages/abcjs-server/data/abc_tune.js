...
/**
 * This is the data for a single ABC tune. It is created and populated by the ABCJS.parse.Parse class.
...
ABCJS.data.Tune = function() {
	...
	this.cleanUp = function(defWidth, defLength, barsperstaff, staffnonote, currSlur) {
		...
		if (anyDeleted) {
			...
			ABCJS.parse.each(this.lines, function(line) {
				if (line.staff)
					line.staff = ABCJS.parse.compact(line.staff);
			});
		}
									...
									if (barNumThisLine >= barsperstaff) {
										...
										if (n < this.lines[i].staff[s].voices[v].length - 1) {
											if (i === this.lines.length - 1) {
												...
												this.lines.push(ABCJS.parse.clone(cp));
			...
			var addEndSlur = function(obj, num, chordPos) {
				if (currSlur[chordPos] === undefined) {
					...
					if (currSlur[chordPos] === undefined) {
						...
						ABCJS.parse.each(obj.endSlur, function(x) { if (offNum === x) --offNum; });
...
//					lyr.syllable += '<' + slurNum;	// TODO-PER: debugging
			...
			var addStartSlur = function(obj, num, chordPos, usedNums) {
				...
				for (var i = 0; i < num; i++) {
					if (usedNums) {
						ABCJS.parse.each(usedNums, function(x) { if (nextNum === x) ++nextNum; });
						ABCJS.parse.each(usedNums, function(x) { if (nextNum === x) ++nextNum; });
						ABCJS.parse.each(usedNums, function(x) { if (nextNum === x) ++nextNum; });
					}
					ABCJS.parse.each(currSlur[chordPos], function(x) { if (nextNum === x) ++nextNum; });
					ABCJS.parse.each(currSlur[chordPos], function(x) { if (nextNum === x) ++nextNum; });
...
//				lyr.syllable = '';	// TODO-PER: debugging
		...
		function fixClefPlacement(el) {
			ABCJS.parse.parseKeyVoice.fixClef(el);
			//if (el.el_type === 'clef') {
//				var min = -2;
//				var max = 5;
//				switch(el.type) {
	...
	this.appendElement = function(type, startChar, endChar, hashParams)
	{
		...
		var pushNote = function(hp) {
			if (hp.pitches !== undefined) {
				...
				ABCJS.parse.each(hp.pitches, function(p) { p.verticalPos = p.pitch - mid; });
			}
			if (hp.gracenotes !== undefined) {
				var mid2 = This.lines[This.lineNum].staff[This.staffNum].workingClef.verticalPos;
				ABCJS.parse.each(hp.gracenotes, function(p) { p.verticalPos = p.pitch - mid2; });
			}
...
//			}
	...
	this.appendStartingElement = function(type, startChar, endChar, hashParams2)
	{
		...
		var hashParams = ABCJS.parse.clone(hashParams2);
