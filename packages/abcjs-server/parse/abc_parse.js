...
ABCJS.parse.Parse = function() {
	"use strict";
	var tune = new ABCJS.data.Tune();
	var tokenizer = new ABCJS.parse.tokenizer();
	...
	var encode = function(str) {
		var ret = ABCJS.parse.gsub(str, '\x12', ' ');
		ret = ABCJS.parse.gsub(ret, '&', '&amp;');
		ret = ABCJS.parse.gsub(ret, '<', '&lt;');
		return ABCJS.parse.gsub(ret, '>', '&gt;');
	};
	...
	var header = new ABCJS.parse.ParseHeader(tokenizer, warn, multilineVars, tune);
	...
	var letter_to_accent = function(line, i)
	{
		...
		if (macro !== undefined) {
			...
			if (ABCJS.parse.detect(legalAccents, function(acc) {
					return (macro === acc);
				}))
				return [ 1, macro ];
			else if (ABCJS.parse.detect(volumeDecorations, function(acc) {
					return (macro === acc);
				})) {
				if (multilineVars.volumePosition === 'hidden')
					macro = "";
				return [1, macro];
			} else if (ABCJS.parse.detect(dynamicDecorations, function(acc) {
				...
				if (!ABCJS.parse.detect(multilineVars.ignoredDecorations, function(dec) {
					return (macro === dec);
				}))
		...
		switch (line.charAt(i))
		{
			...
			if (ret[1].length > 0 && (ret[1].charAt(0) === '^' || ret[1].charAt(0) ==='_'))
				...
				if (ABCJS.parse.detect(legalAccents, function(acc) {
					...
					return ret;
				if (ABCJS.parse.detect(volumeDecorations, function(acc) {
						return (ret[1] === acc);
					})) {
					if (multilineVars.volumePosition === 'hidden' )
						ret[1] = '';
				...
				if (ABCJS.parse.detect(dynamicDecorations, function(acc) {
						return (ret[1] === acc);
					})) {
					if (multilineVars.dynamicPosition === 'hidden' )
						ret[1] = '';
				...
				if (ABCJS.parse.detect(accentPseudonyms, function(acc) {
					if (ret[1] === acc[0]) {
						ret[1] = acc[1];
						return true;
					} else
				...
				if (ABCJS.parse.detect(accentDynamicPseudonyms, function(acc) {
					if (ret[1] === acc[0]) {
						ret[1] = acc[1];
						return true;
					} else
	...
	var addWords = function(line, words) {
		...
		var addWord = function(i) {
			var word = ABCJS.parse.strip(words.substring(last_divider, i));
			last_divider = i+1;
			if (word.length > 0) {
				if (replace)
					word = ABCJS.parse.gsub(word,'~', ' ');
			...
			switch (words.charAt(i)) {
				...
				case '-':
					if (!addWord(i) && word_list.length > 0) {
						ABCJS.parse.last(word_list).divider = '-';
		...
		var inSlur = false;
		ABCJS.parse.each(line, function(el) {
	...
	function startNewLine() {
		...
		params.clef = multilineVars.currentVoice && multilineVars.staves[multilineVars.currentVoice.staffNum].clef !== undefined ? ABCJS.parse.clone(multilineVars.staves[multilineVars.currentVoice.staffNum].clef) : ABCJS.parse.clone(multilineVars.clef) ;
		params.key = ABCJS.parse.parseKeyVoice.deepCopyKey(multilineVars.key);
		ABCJS.parse.parseKeyVoice.addPosToKey(params.clef, params.key);
		if (multilineVars.meter !== null) {
			if (multilineVars.currentVoice) {
				ABCJS.parse.each(multilineVars.staves, function(st) {
							...
							if (chordNote !== null) {
								...
								if (i < line.length && line.charAt(i) === ']') {
									...
									if (multilineVars.inTie) {
										ABCJS.parse.each(el.pitches, function(pitch) { pitch.endTie = true; });
									...
									while (i < line.length && !postChordDone) {
										switch (line.charAt(i)) {
											...
											case ')':
												...
												break;
											case '-':
												ABCJS.parse.each(el.pitches, function(pitch) { pitch.startTie = {}; });
	...
	this.parse = function(strTune, switches) {
		...
		strTune = ABCJS.parse.gsub(strTune, '\r\n', '\n');
		strTune = ABCJS.parse.gsub(strTune, '\r', '\n');
		...
		if (ABCJS.parse.last(lines).length === 0)	// remove the blank line we added above.
			lines.pop();
		try {
			if (switches.format) {
				ABCJS.parse.parseDirective.globalFormatting(switches.format);
			}
			ABCJS.parse.each(lines,  function(line) {
