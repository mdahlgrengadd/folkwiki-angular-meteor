...
(function() {
	...
	var multilineVars;
	var tune;
	ABCJS.parse.parseDirective.initialize = function(tokenizer_, warn_, multilineVars_, tune_) {
	...
	var setScale = function(cmd, tokens) {
		var scratch = "";
		ABCJS.parse.each(tokens, function(tok) {
			scratch += tok.token;
		});
	...
	ABCJS.parse.parseDirective.parseFontChangeLine = function(textstr) {
	...
	ABCJS.parse.parseDirective.addDirective = function(str) {
		...
		switch (cmd)
		{
			...
			case "text":
				...
				tune.addText(ABCJS.parse.parseDirective.parseFontChangeLine(textstr));
				break;
			case "center":
				var centerstr = tokenizer.translateString(restOfString);
				tune.addCentered(ABCJS.parse.parseDirective.parseFontChangeLine(centerstr));
				break;
			case "font":
...
//					return "Bad parameters: " + cmd;
			...
			case "score":
				...
				var addVoice = function(id, newStaff, bracket, brace, continueBar) {
					...
					var staff = ABCJS.parse.last(multilineVars.staves);
	...
	ABCJS.parse.parseDirective.globalFormatting = function(formatHash) {
