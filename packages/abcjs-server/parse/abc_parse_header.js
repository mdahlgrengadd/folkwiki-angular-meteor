...
ABCJS.parse.ParseHeader = function(tokenizer, warn, multilineVars, tune) {
	this.reset = function(tokenizer, warn, multilineVars, tune) {
		ABCJS.parse.parseKeyVoice.initialize(tokenizer, warn, multilineVars, tune);
		ABCJS.parse.parseDirective.initialize(tokenizer, warn, multilineVars, tune);
	};
	...
	this.addUserDefinition = function(line, start, end) {
		...
		var before = ABCJS.parse.strip(line.substring(start, equals));
		var after = ABCJS.parse.strip(line.substring(equals+1));
	...
	this.setDefaultLength = function(line, start, end) {
		var len = ABCJS.parse.gsub(line.substring(start, end), " ", "");
	...
	this.letter_to_inline_header = function(line, i)
	{
		...
		if (line.length >= i+5 && line.charAt(i) === '[' && line.charAt(i+2) === ':') {
			...
			{
				case "[I:":
					var err = ABCJS.parse.parseDirective.addDirective(line.substring(i+3, e));
				...
				case "[M:":
					...
					return [ e-i+1+ws ];
				case "[K:":
					var result = ABCJS.parse.parseKeyVoice.parseKey(line.substring(i+3, e));
					...
					if (result.foundKey && tune.hasBeginMusic())
						tune.appendStartingElement('key', -1, -1, ABCJS.parse.parseKeyVoice.fixKey(multilineVars.clef, multilineVars.key));
					return [ e-i+1+ws ];
				case "[P:":
				...
				case "[V:":
					if (e > 0) {
						ABCJS.parse.parseKeyVoice.parseVoice(line, i+3, e);
						//startNewLine();
						return [ e-i+1+ws, line.charAt(i+1), line.substring(i+3, e)];
					}
					break;
	...
	this.letter_to_body_header = function(line, i)
	{
		if (line.length >= i+3) {
			...
			{
				case "I:":
					var err = ABCJS.parse.parseDirective.addDirective(line.substring(i+2));
					if (err) warn(err, line, i);
					return [ line.length ];
				case "M:":
					...
					return [ line.length ];
				case "K:":
					var result = ABCJS.parse.parseKeyVoice.parseKey(line.substring(i+2));
					if (result.foundClef && tune.hasBeginMusic())
						tune.appendStartingElement('clef', -1, -1, multilineVars.clef);
					if (result.foundKey && tune.hasBeginMusic())
						tune.appendStartingElement('key', -1, -1, ABCJS.parse.parseKeyVoice.fixKey(multilineVars.clef, multilineVars.key));
				...
				return [ e, line.charAt(i), ABCJS.parse.strip(line.substring(i+2))];
				case "V:":
					ABCJS.parse.parseKeyVoice.parseVoice(line, 2, line.length);
//						startNewLine();
					return [ line.length, line.charAt(i), ABCJS.parse(line.substring(i+2))];
				default:
					// TODO: complain about unhandled header
			}
		}
	...
	this.parseHeader = function(line) {
		if (ABCJS.parse.startsWith(line, '%%')) {
			var err = ABCJS.parse.parseDirective.addDirective(line.substring(2));
			...
			if (line.charAt(1) === ':') {
				...
				if (field !== undefined) {
					if (field === 'unalignedWords')
						tune.addMetaTextArray(field, ABCJS.parse.parseDirective.parseFontChangeLine(tokenizer.translateString(tokenizer.stripComment(line.substring(2)))));
					else
					...
					switch(line.charAt(0))
					{
						...
						case  'K':
							...
							this.resolveTempo();
							var result = ABCJS.parse.parseKeyVoice.parseKey(line.substring(2));
