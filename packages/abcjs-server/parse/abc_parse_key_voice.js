...
(function() {
	...
	var multilineVars;
	var tune;
	ABCJS.parse.parseKeyVoice.initialize = function(tokenizer_, warn_, multilineVars_, tune_) {
	...
	ABCJS.parse.parseKeyVoice.standardKey = function(keyName) {
	...
	ABCJS.parse.parseKeyVoice.fixClef = function(clef) {
	...
	ABCJS.parse.parseKeyVoice.deepCopyKey = function(key) {
		var ret = { accidentals: [], root: key.root, acc: key.acc, mode: key.mode };
		ABCJS.parse.each(key.accidentals, function(k) {
		ret.accidentals.push(ABCJS.parse.clone(k));
		});
		return ret;
	};
	...
	ABCJS.parse.parseKeyVoice.addPosToKey = function(clef, key) {
		...
		ABCJS.parse.each(key.accidentals, function(acc) {
			var pitch = pitches[acc.note];
			pitch = pitch - mid;
			acc.verticalPos = pitch;
		});
		if (key.impliedNaturals)
			ABCJS.parse.each(key.impliedNaturals, function(acc) {
				var pitch = pitches[acc.note];
				pitch = pitch - mid;
				acc.verticalPos = pitch;
			});
	...
	ABCJS.parse.parseKeyVoice.fixKey = function(clef, key) {
		var fixedKey = ABCJS.parse.clone(key);
		ABCJS.parse.parseKeyVoice.addPosToKey(clef, fixedKey);
		return fixedKey;
	};
	...
	ABCJS.parse.parseKeyVoice.parseKey = function(str)	// (and clef)
	{
		// returns:
		...
		switch (tokens[0].token) {
			...
			default:
				...
				if (retPitch.len > 0) {
					...
					var oldKey = ABCJS.parse.parseKeyVoice.deepCopyKey(multilineVars.key);
					multilineVars.key = ABCJS.parse.parseKeyVoice.deepCopyKey({accidentals: ABCJS.parse.parseKeyVoice.standardKey(key)});
	...
	ABCJS.parse.parseKeyVoice.parseVoice = function(line, i, e) {
