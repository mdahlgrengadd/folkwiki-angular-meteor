...
//			case 'g':return {len: i+1, token: 'G'};
	...
	this.getClef = function(str, bExplicitOnly) {
		...
		if (ABCJS.parse.startsWith(strClef, 'clef=')) {
			needsClef = true;
		...
		var name = null;
		if (ABCJS.parse.startsWith(strClef, 'treble'))
			name = 'treble';
		else if (ABCJS.parse.startsWith(strClef, 'bass3'))
			name = 'bass3';
		else if (ABCJS.parse.startsWith(strClef, 'bass'))
			name = 'bass';
		else if (ABCJS.parse.startsWith(strClef, 'tenor'))
			name = 'tenor';
		else if (ABCJS.parse.startsWith(strClef, 'alto2'))
			name = 'alto2';
		else if (ABCJS.parse.startsWith(strClef, 'alto1'))
			name = 'alto1';
		else if (ABCJS.parse.startsWith(strClef, 'alto'))
			name = 'alto';
		else if (!bExplicitOnly && (needsClef && ABCJS.parse.startsWith(strClef, 'none')))
			name = 'none';
		else if (ABCJS.parse.startsWith(strClef, 'perc'))
			name = 'perc';
		else if (!bExplicitOnly && (needsClef && ABCJS.parse.startsWith(strClef, 'C')))
			name = 'tenor';
		else if (!bExplicitOnly && (needsClef && ABCJS.parse.startsWith(strClef, 'F')))
			name = 'bass';
		else if (!bExplicitOnly && (needsClef && ABCJS.parse.startsWith(strClef, 'G')))
			name = 'treble';
		else
	...
	this.isMatch = function(str, match) {
		...
		if (finished(str, i))
			return 0;
		if (ABCJS.parse.startsWith(str.substring(i), match))
...
// More chars: Ñ Ĳ ĳ Ď ď Đ đ Ĝ ĝ Ğ ğ Ġ ġ Ģ ģ Ĥ ĥ Ħ ħ Ĵ ĵ Ķ ķ ĸ Ĺ ĺ Ļ ļ Ľ ľ Ŀ ŀ Ł ł Ń ń Ņ ņ Ň ň ŉ Ŋ ŋ   Ŕ ŕ Ŗ ŗ Ř ř Ś ś Ŝ ŝ Ş ş Š Ţ ţ Ť ť Ŧ ŧ Ŵ ŵ Ŷ ŷ Ÿ ÿ Ÿ Ź ź Ż ż Ž 
	...
	this.translateString = function(str) {
		...
		var out = null;
		ABCJS.parse.each(arr, function(s) {
			if (out === null)
				out = s;
			else {
	...
	this.theReverser = function(str) {
		if (ABCJS.parse.endsWith(str, ", The"))
			return "The " + str.substring(0, str.length-5);
		if (ABCJS.parse.endsWith(str, ", A"))
	...
	this.stripComment = function(str) {
		var i = str.indexOf('%');
		if (i >= 0)
			return ABCJS.parse.strip(str.substring(0, i));
		return ABCJS.parse.strip(str);
	};
