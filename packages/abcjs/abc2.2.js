/*! abcjs v2.2 Copyright © 2009-2015 Paul Rosen and Gregory Dyke (http://abcjs.net) */
(function(i) {
    var e = "0.4.2",
        j = "hasOwnProperty",
        b = /[\.\/]/,
...
(function(b, a) {
    ...
    function H(i, d, b) {
        function g() {
            var E = Array.prototype.slice.call(arguments, 0),
                S = E.join("\u2400"),
                R = g.cache = g.cache || {}, bO = g.count = g.count || [];
            if (R[bw](S)) {
                c(bO, S);
                return b ? b(R[S]) : R[S]
            }
    ...
    var I = bi.pathBBox = function(bY) {
        ...
        bk = bi._path2curve = H(function(bX, bS) {
            ...
            for (var bP = 0, bV = bI(E.length, bT && bT.length || 0);
            bP < bV; bP++) {
                E[bP] = S(E[bP], bU);
                b(E, bP);
                bT && (bT[bP] = S(bT[bP], d));
    ...
    var aZ = {}, aO = function(d, b, E, i, bP, bO, S, R, g) {
        if (g == null) {
        ...
        if (bS) {
            ...
            for (var bX = 0, bQ = b1.length; bX < bQ; bX++) {
                if (b1[bX] == "\n") {
                    ...
                    var bV = b0 && bS.glyphs[b1[bX - 1]] || {}, bZ = bS.glyphs[b1[bX]];
        ...
        function l(K) {
            ...
            var D = function(N, j, O, M) {
                ...
                for (var L = 0; L < j; L++) {
                    ...
                    ABCJS.parse.each(w[O], function(i) {
                        if (P === i) {
                            ++P
                        }
                    });
...
ABCJS.parse.Parse = function() {
    var g = new ABCJS.data.Tune();
    var r = new ABCJS.parse.tokenizer();
    ...
    var d = function(H) {
        var G = ABCJS.parse.gsub(H, "\x12", " ");
        G = ABCJS.parse.gsub(G, "&", "&amp;");
        G = ABCJS.parse.gsub(G, "<", "&lt;");
        return ABCJS.parse.gsub(G, ">", "&gt;")
    };
    ...
    var y = new ABCJS.parse.ParseHeader(r, B, C, g);
    ...
    var k = function(G, I) {
        ...
        if (J !== undefined) {
            ...
            if (ABCJS.parse.detect(t, function(K) {
                return (J === K)
            })) {
                return [1, J]
            } else {
                if (ABCJS.parse.detect(E, function(K) {
                    return (J === K)
                })) {
                    ...
                    if (ABCJS.parse.detect(b, function(K) {
                        ...
                        if (!ABCJS.parse.detect(C.ignoredDecorations, function(K) {
                            return (J === K)
                        })) {
        ...
        switch (G.charAt(I)) {
            ...
            case "+":
                ...
                if (ABCJS.parse.detect(v, function(K) {
                    if (H[1] === K[0]) {
                        H[1] = K[1];
                        return true
                    } else {
                ...
                if (ABCJS.parse.detect(i, function(K) {
                    if (H[1] === K[0]) {
                        H[1] = K[1];
                        return true
                    } else {
    ...
    var q = function(H, N) {
        ...
        var J = false;
        var G = function(O) {
            var P = ABCJS.parse.strip(N.substring(L, O));
            L = O + 1;
            if (P.length > 0) {
                if (J) {
                    P = ABCJS.parse.gsub(P, "~", " ")
                }
                var Q = N.charAt(O);
                if (Q !== "_" && Q !== "-") {
                    Q = " "
                ...
                case "g":
                    if (H === "startSlur" || H === "sharp2" || H === "flat2" || H === "pitch") {
                        ...
                        if (G && C.next_note_duration !== 0) {
                            I.duration = C.default_length * C.next_note_duration;
    ...
    function z() {
        ...
        H.clef = C.currentVoice && C.staves[C.currentVoice.staffNum].clef !== undefined ? ABCJS.parse.clone(C.staves[C.currentVoice.staffNum].clef) : ABCJS.parse.clone(C.clef);
        H.key = ABCJS.parse.parseKeyVoice.deepCopyKey(C.key);
        ABCJS.parse.parseKeyVoice.addPosToKey(H.clef, H.key);
        if (C.meter !== null) {
            if (C.currentVoice) {
                ABCJS.parse.each(C.staves, function(I) {
                    I.meter = C.meter
                });
    ...
    var p = function(Q) {
        ...
        var Y = 0;
        var X = C.iChar;
        while (r.isWhiteSpace(Q.charAt(Y)) && Y < Q.length) {
            Y++
        }
        if (Y === Q.length || Q.charAt(Y) === "%") {
            return
        }
        var ab = C.start_new_line;
        ...
        var N = 0;
        var T = y.letter_to_body_header(Q, Y);
        if (T[0] > 0) {
            Y += T[0]
        }
        var I = {};
        while (Y < Q.length) {
            ...
            var H = y.letter_to_inline_header(Q, Y);
            if (H[0] > 0) {
                Y += H[0]
            } else {
                if (ab) {
                ...
                var ad;
                while (1) {
                    ad = r.eatWhiteSpace(Q, Y);
                    if (ad > 0) {
                        Y += ad
                    }
                    if (Y > 0 && Q.charAt(Y - 1) === "\x12") {
                        ad = y.letter_to_body_header(Q, Y);
                        if (ad[0] > 0) {
                            Y = ad[0];
                            C.start_new_line = false
                        }
                    ...
                    if (ad[0] > 0) {
                        ...
                        var M = r.translateString(ad[1]);
                        M = M.replace(/;/g, "\n");
                        var L = false;
                        for (var R = 0; R < I.chord.length; R++) {
                            if (I.chord[R].position === ad[2]) {
                                L = true;
                                I.chord[R].name += "\n" + M
                            }
                        }
                        ...
                        Y += ad[0];
                        var S = r.skipWhiteSpace(Q.substring(Y));
                        if (S > 0) {
                            I.force_end_beam_last = true
                        }
                        Y += S
                    } else {
                        if (u.indexOf(Q.charAt(Y)) === -1) {
                            ad = k(Q, Y)
                        } else {
                            ad = [0]
                        }
                        if (ad[0] > 0) {
                            if (ad[1] === null) {
                                ...
                                if (ad[1].length > 0) {
                                    if (ad[1].indexOf("style=") === 0) {
                                        I.style = ad[1].substr(6)
                                    } else {
                                        if (I.decoration === undefined) {
                                            I.decoration = []
                                        }
                                        I.decoration.push(ad[1])
                                    }
                                }
                            }
                            Y += ad[0]
                ...
                if (ad[0] > 0) {
                    if (I.gracenotes !== undefined) {
                        ...
                        g.appendElement("note", X + Y, X + Y + ad[0], I);
                    ...
                    if (aa.type.length === 0) {
                        B("Unknown bar type", Q, Y)
                    } else {
                        if (C.inEnding && aa.type !== "bar_thin") {
                            aa.endEnding = true;
                            C.inEnding = false
                        }
                        if (ad[2]) {
                        ...
                        if (I.decoration !== undefined) {
                            aa.decoration = I.decoration
                        }
                        if (I.chord !== undefined) {
                            aa.chord = I.chord
                        }
                        if (aa.startEnding && C.barFirstEndingNum === undefined) {
                            C.barFirstEndingNum = C.currBarNumber
                        } else {
                            if (aa.startEnding && aa.endEnding && C.barFirstEndingNum) {
                        ...
                        if (aa.type !== "bar_invisible" && C.measureNotEmpty) {
                            C.currBarNumber++;
                        ...
                        g.appendElement("bar", X + Y, X + Y + ad[0], aa);
                    ...
                    if (Q[Y] === "&") {
                        B("Overlay not yet supported", Q, Y);
                        Y++
                    } else {
                        ad = s(Q, Y);
                        if (ad.consumed > 0) {
                            if (ad.startSlur !== undefined) {
                                I.startSlur = ad.startSlur
                            }
                            if (ad.triplet !== undefined) {
                                if (N > 0) {
                                    B("Can't nest triplets", Q, Y)
                                } else {
                                    I.startTriplet = ad.triplet;
                                    N = ad.num_notes === undefined ? ad.triplet : ad.num_notes
                                }
                            }
                            Y += ad.consumed
                        }
                        if (Q.charAt(Y) === "[") {
                            ...
                            while (!V) {
                                ...
                                if (P !== null) {
                                    ...
                                    if (Q.charAt(Y) === " ") {
                                        B("Spaces are not allowed in chords", Q, Y);
                                        Y++
                                    } else {
                                        if (Y < Q.length && Q.charAt(Y) === "]") {
                                            Y++;
                                            ...
                                            if (C.inTie) {
                                                ABCJS.parse.each(I.pitches, function(ae) {
                                                    ae.endTie = true
                                                });
                                                C.inTie = false
                                            }
                                            ...
                                            while (Y < Q.length && !O) {
                                                switch (Q.charAt(Y)) {
                                                    ...
                                                    case "9":
                                                    case "/":
                                                        var K = r.getFraction(Q, Y);
                                                        G = K.value;
                                                        Y = K.index;
                                                        if (Q.charAt(Y) === "-" || Q.charAt(Y) === ")" || Q.charAt(Y) === " " || Q.charAt(Y) === "<" || Q.charAt(Y) === ">") {
                                                            Y--
                                                        } else {
                                                            O = true
                                                        }
                                            ...
                                            B("Expected ']' to end the chords", Q, Y)
                                        }
                                        if (I.pitches !== undefined) {
                                            if (G !== null) {
                                            ...
                                            C.addFormattingOptions(I, g.formatting, "note");
                                            g.appendElement("note", X + W, X + Y, I);
                                            C.measureNotEmpty = true;
                                            I = {}
                                        }
                                        V = true
                            ...
                            if (Z !== null) {
                                if (Z.pitch !== undefined) {
                                    I.pitches = [{}];
                                    if (Z.accidental !== undefined) {
                                        I.pitches[0].accidental = Z.accidental
                                    }
                                    I.pitches[0].pitch = Z.pitch;
                                    if (Z.endSlur !== undefined) {
                                        I.pitches[0].endSlur = Z.endSlur
                                    }
                                    if (Z.endTie !== undefined) {
                                        I.pitches[0].endTie = Z.endTie
                                    }
                                    if (Z.startSlur !== undefined) {
                                        I.pitches[0].startSlur = Z.startSlur
                                    }
                                    if (I.startSlur !== undefined) {
                                        I.pitches[0].startSlur = I.startSlur
                                    }
                                    if (Z.startTie !== undefined) {
                                        I.pitches[0].startTie = Z.startTie
                                    }
                                    ...
                                    if (Z.endTie !== undefined) {
                                        I.rest.endTie = Z.endTie
                                    }
                                    if (Z.startSlur !== undefined) {
                                        I.startSlur = Z.startSlur
                                    }
                                    if (Z.startTie !== undefined) {
                                        I.rest.startTie = Z.startTie
                                    }
                                    if (I.startTie !== undefined) {
                                        I.rest.startTie = I.startTie
                                    }
                                }
                                if (Z.chord !== undefined) {
                                    I.chord = Z.chord
                                }
                                if (Z.duration !== undefined) {
                                    I.duration = Z.duration
                                }
                                if (Z.decoration !== undefined) {
                                    I.decoration = Z.decoration
                                }
                                if (Z.graceNotes !== undefined) {
                                    I.graceNotes = Z.graceNotes
                                }
                                delete I.startSlur;
                                if (C.inTie) {
                                    if (I.pitches !== undefined) {
                                ...
                                C.addFormattingOptions(I, g.formatting, "note");
                                g.appendElement("note", X + J, X + Y, I);
                                C.measureNotEmpty = true;
                                I = {}
                            }
                        }
                        if (Y === J) {
                            if (Q.charAt(Y) !== " " && Q.charAt(Y) !== "`") {
                                B("Unknown character ignored", Q, Y)
                            }
                            Y++
                        }
                    }
    ...
    this.parse = function(M, K) {
        ...
        C.reset();
        y.reset(r, B, C, g);
        M = ABCJS.parse.gsub(M, "\r\n", "\n");
        M = ABCJS.parse.gsub(M, "\r", "\n");
        M += "\n";
        M = M.replace(/\n\\.*\n/g, "\n");
        ...
        M = M.replace(/\\([ \t]*)(%.*)*\n/g, I);
        var H = M.split("\n");
        if (ABCJS.parse.last(H).length === 0) {
            H.pop()
        }
        try {
            if (K.format) {
                ABCJS.parse.parseDirective.globalFormatting(K.format)
            }
            ABCJS.parse.each(H, function(O) {
                ...
                if (C.is_in_history) {
                    ...
                    if (C.inTextBlock) {
                        if (ABCJS.parse.startsWith(O, "%%endtext")) {
                            g.addText(C.textBlock);
                            C.inTextBlock = false
                        } else {
                            if (ABCJS.parse.startsWith(O, "%%")) {
                        ...
                        if (C.inPsBlock) {
                            if (ABCJS.parse.startsWith(O, "%%endps")) {
                                C.inPsBlock = false
                            } else {
                                C.textBlock += " " + O
                            }
...
(function() {
    ...
    var e;
    var g;
    ABCJS.parse.parseDirective.initialize = function(u, s, t, r) {
        p = u;
        j = s;
        e = t;
        g = r;
    ...
    ABCJS.parse.parseDirective.parseFontChangeLine = function(t) {
        var r = t.split("$");
    ...
    ABCJS.parse.parseDirective.addDirective = function(R) {
        ...
        switch (ac) {
            ...
            case "text":
                ...
                g.addText(ABCJS.parse.parseDirective.parseFontChangeLine(Z));
                break;
            case "center":
                var r = p.translateString(s);
                g.addCentered(ABCJS.parse.parseDirective.parseFontChangeLine(r));
                break;
            case "font":
                break;
            case "setfont":
            ...
            case "score":
                ...
                var ag = function(am, aj, al, ak, ai) {
                    ...
                    var t = ABCJS.parse.last(e.staves);
                    if (al !== undefined) {
                        t.bracket = al
                    }
                    if (ak !== undefined) {
    ...
    ABCJS.parse.parseDirective.globalFormatting = function(v) {
        for (var t in v) {
...
ABCJS.write.AbsoluteElement.prototype.addCentered = function(a) {
    var b = a.w / 2;
    if (-b < this.extraw) {
        this.extraw = -b
    }
...
if (!ABCJS.write) {
    ...
    ABCJS.write.AbstractEngraver.prototype.createABCElement = function() {
        ...
        switch (d.el_type) {
            ...
            case "part":
                ...
                var g = this.renderer.getTextSize(d.title, "partsfont", "part");
                a.addChild(new ABCJS.write.RelativeElement(d.title, 0, 0, undefined, {
                    type: "part",
                    height: g.height / ABCJS.write.spacing.STEP
                }));
                f[0] = a;
                break;
            case "tempo":
    ...
    ABCJS.write.AbstractEngraver.prototype.createNote = function(X, e, af) {
        var v = null;
        var b = null;
        this.roomtaken = 0;
        ...
        var S, ab, r;
        var d, ae, ad, g;
        var A = ABCJS.write.getDuration(X);
        var R = false;
        if (A === 0) {
            R = true;
            A = 0.25;
        ...
        var F = Math.floor(Math.log(A) / Math.log(2));
        var ak = 0;
        for (var h = Math.pow(2, F), ac = h / 2; h < A; ak++, h += ac, ac /= 2) {}
        if (X.startTriplet) {
            if (X.startTriplet === 2) {
                this.tripletmultiplier = 3 / 2
            } else {
                this.tripletmultiplier = (X.startTriplet - 1) / X.startTriplet
            }
        }
        var k = new ABCJS.write.AbsoluteElement(X, A * this.tripletmultiplier, 1, "note");
        if (X.rest) {
            var q = 7;
            ...
            var E = this.staffgroup.staffs[this.staffgroup.staffs.length - 1].lines;
            if (E === 1) {
                if (A < 0.5) {
                    q = 7
                } else {
            ...
            if (!af) {
                v = this.createNoteHead(k, ai, {
                    verticalPos: q
                }, null, 0, -this.roomtaken, null, ak, 0, 1)
            }
            if (v) {
                k.addHead(v)
            }
            ...
            ABCJS.write.sortPitch(X);
            var t = 0;
            for (S = 0, r = X.pitches.length; S < r; S++) {
                t += X.pitches[S].verticalPos
            }
            X.averagepitch = t / X.pitches.length;
            X.minpitch = X.pitches[0].verticalPos;
            this.minY = Math.min(X.minpitch, this.minY);
            X.maxpitch = X.pitches[X.pitches.length - 1].verticalPos;
            var aj = (X.averagepitch >= 6) ? "down" : "up";
            if (this.stemdir) {
                aj = this.stemdir
            }
            var T = X.style ? X.style : this.style;
            if (!T || T === "normal") {
                T = "note"
            }
            var m;
            if (R) {
                m = this.chartable[T].nostem
            } else {
                m = this.chartable[T][-F]
            }
            if (!m) {
                console.log("noteSymbol:", T, F, R)
            }
            for (S = (aj === "down") ? X.pitches.length - 2 : 1;
            (aj === "down") ? S >= 0 : S < X.pitches.length; S = (aj === "down") ? S - 1 : S + 1) {
                var G = X.pitches[(aj === "down") ? S + 1 : S - 1];
                var M = X.pitches[S];
                var N = (aj === "down") ? G.pitch - M.pitch : M.pitch - G.pitch;
                if (N <= 1 && !G.printer_shift) {
                    M.printer_shift = (N) ? "different" : "same";
                    if (M.verticalPos > 11 || M.verticalPos < 1) {
                        w.push(M.verticalPos - (M.verticalPos % 2))
                    }
                    if (aj === "down") {
                        this.roomtaken = ABCJS.write.glyphs.getSymbolWidth(m) + 2
                    } else {
                        L = ABCJS.write.glyphs.getSymbolWidth(m) + 2
                    }
                }
            }
            this.accidentalSlot = [];
            for (S = 0; S < X.pitches.length; S++) {
                if (!e) {
                    if ((aj === "down" && S !== 0) || (aj === "up" && S !== r - 1)) {
                        J = null
                    } else {
                        J = this.chartable[(aj === "down") ? "dflags" : "uflags"][-F]
                    }
                }
                ai = m;
                X.pitches[S].highestVert = X.pitches[S].verticalPos;
                var o = (this.stemdir === "up" || aj === "up") && S === 0;
                var U = (this.stemdir === "down" || aj === "down") && S === r - 1;
                if (!af && (o || U)) {
                    if (X.startSlur || r === 1) {
                        X.pitches[S].highestVert = X.pitches[r - 1].verticalPos;
                        if (this.stemdir === "up" || aj === "up") {
                            X.pitches[S].highestVert += 6
                        }
                    }
                    if (X.startSlur) {
                        if (!X.pitches[S].startSlur) {
                            X.pitches[S].startSlur = []
                        }
                        for (ab = 0; ab < X.startSlur.length; ab++) {
                            X.pitches[S].startSlur.push(X.startSlur[ab])
                        }
                    }
                    if (!af && X.endSlur) {
                        X.pitches[S].highestVert = X.pitches[r - 1].verticalPos;
                        ...
                        if (!X.pitches[S].endSlur) {
                            X.pitches[S].endSlur = []
                        }
                        for (ab = 0; ab < X.endSlur.length; ab++) {
                            X.pitches[S].endSlur.push(X.endSlur[ab])
                        }
                    }
                }
                var u = !e && F <= -1;
                if (!af) {
                    v = this.createNoteHead(k, ai, X.pitches[S], u ? aj : null, 0, -this.roomtaken, J, ak, L, 1)
                }
                if (v) {
                    k.addHead(v)
                }
            ...
            if (u) {
                ae = (aj === "down") ? X.minpitch - 7 : X.minpitch + 1 / 3;
                if (ae > 6 && !this.stemdir) {
                    ae = 6
                }
                ad = (aj === "down") ? X.maxpitch - 1 / 3 : X.maxpitch + 7;
                if (ad < 6 && !this.stemdir) {
                    ad = 6
                }
                g = (aj === "down" || k.heads.length === 0) ? 0 : k.heads[0].w;
                d = (aj === "down") ? 1 : -1;
                if (v.c === "noteheads.slash.quarter") {
                    if (aj === "down") {
                        ad -= 1
                    } else {
                        ae += 1
                ...
                k.addExtra(new ABCJS.write.RelativeElement(null, g, 0, ae, {
                    type: "stem",
                    pitch2: ad,
                    linewidth: d
                }));
                this.minY = Math.min(ae, this.minY);
                this.minY = Math.min(ad, this.minY)
            }
        }
        if (X.lyric !== undefined) {
            var W = "";
            ABCJS.parse.each(X.lyric, function(c) {
                W += c.syllable + c.divider + "\n"
            });
            var l = this.renderer.getTextSize(W, "vocalfont", "abc-lyric");
            var Q = X.positioning ? X.positioning.vocalPosition : "below";
            k.addCentered(new ABCJS.write.RelativeElement(W, 0, l.width, undefined, {
                type: "lyric",
                position: Q,
                height: l.height / ABCJS.write.spacing.STEP
            }))
        }
        if (!af && X.gracenotes !== undefined) {
            var D = 3 / 5;
            var z = 3.5 / 5;
            var n = null;
            if (X.gracenotes.length > 1) {
                n = new ABCJS.write.BeamElem(this.stemHeight * z, "grace", this.isBagpipes);
                n.mainNote = k
            }
            var ah = [];
            for (ab = X.gracenotes.length - 1; ab >= 0; ab--) {
                this.roomtaken += 10;
                ah[ab] = this.roomtaken;
                if (X.gracenotes[ab].accidental) {
                    this.roomtaken += 7
                }
            }
            for (ab = 0; ab < X.gracenotes.length; ab++) {
                var B = X.gracenotes[ab].verticalPos;
                J = (n) ? null : this.chartable.uflags[(this.isBagpipes) ? 5 : 3];
                b = this.createNoteHead(k, "noteheads.quarter", X.gracenotes[ab], "up", -ah[ab], -ah[ab], J, 0, 0, D);
                k.addExtra(b);
                if (X.gracenotes[ab].acciaccatura) {
                    var H = X.gracenotes[ab].verticalPos + 7 * D;
                    var Z = n ? 5 : 6;
                    k.addRight(new ABCJS.write.RelativeElement("flags.ugrace", -ah[ab] + Z, 0, H, {
                        scalex: D,
                        scaley: D
                    }))
                }
                if (n) {
                    var C = X.gracenotes[ab].duration / 2;
                    if (this.isBagpipes) {
                        C /= 2
                    }
                    var s = {
                ...
                if (ab === 0 && !this.isBagpipes && !(X.rest && (X.rest.type === "spacer" || X.rest.type === "invisible"))) {
                    this.voice.addOther(new ABCJS.write.TieElem(b, v, false, true, false))
                }
            }
            if (n) {
                this.voice.addOther(n)
            }
        }
        if (!af && X.decoration) {
            this.decoration.createDecoration(this.voice, X.decoration, k.top, (v) ? v.w : 0, k, this.roomtaken, aj, k.bottom, X.positioning, this.hasVocals)
        }
        if (X.barNumber) {
            k.addChild(new ABCJS.write.RelativeElement(X.barNumber, -10, 0, 0, {
                type: "barNumber"
            }))
        }
        for (ab = X.maxpitch; ab > 11; ab--) {
            if (ab % 2 === 0 && !X.rest) {
                k.addChild(new ABCJS.write.RelativeElement(null, -2, ABCJS.write.glyphs.getSymbolWidth(ai) + 4, ab, {
                    type: "ledger"
                }))
            }
        }
        for (ab = X.minpitch; ab < 1; ab++) {
            if (ab % 2 === 0 && !X.rest) {
                k.addChild(new ABCJS.write.RelativeElement(null, -2, ABCJS.write.glyphs.getSymbolWidth(ai) + 4, ab, {
                    type: "ledger"
                }))
        ...
        for (ab = 0; ab < w.length; ab++) {
            var K = ABCJS.write.glyphs.getSymbolWidth(ai);
            if (aj === "down") {
                K = -K
            }
            k.addChild(new ABCJS.write.RelativeElement(null, K - 2, ABCJS.write.glyphs.getSymbolWidth(ai) + 4, w[ab], {
                type: "ledger"
            }))
        }
        var ag = 8;
        if (X.chord !== undefined) {
            for (ab = 0; ab < X.chord.length; ab++) {
                var P = 0;
                var O;
                var a = this.renderer.getTextSize(X.chord[ab].name, "annotationfont", "annotation");
                var j = a.width;
                var V = a.height / ABCJS.write.spacing.STEP;
                switch (X.chord[ab].position) {
                    case "left":
                        this.roomtaken += j + 7;
                        P = -this.roomtaken;
                        O = X.averagepitch;
                        k.addExtra(new ABCJS.write.RelativeElement(X.chord[ab].name, P, j + 4, O, {
                            type: "text",
                            height: V
                        }));
                        break;
                    case "right":
                        ...
                        P = this.roomtakenright;
                        O = X.averagepitch;
                        k.addRight(new ABCJS.write.RelativeElement(X.chord[ab].name, P, j + 4, O, {
                            type: "text",
                            height: V
                        }));
                        break;
                    case "below":
                        var Y = X.chord[ab].name.split("\n");
                        for (var I = 0; I < Y.length; I++) {
                            k.addRight(new ABCJS.write.RelativeElement(Y[I], P, j + ag, undefined, {
                                type: "text",
                                position: "below",
                                height: V
                            }))
                        ...
                        break;
                    case "above":
                        k.addRight(new ABCJS.write.RelativeElement(X.chord[ab].name, 0, j + ag, undefined, {
                            type: "text",
                            height: V
                        }));
                        break;
                    default:
                        if (X.chord[ab].rel_position) {
                            var aa = X.chord[ab].rel_position.y + 3 * ABCJS.write.spacing.STEP;
                            k.addChild(new ABCJS.write.RelativeElement(X.chord[ab].name, P + X.chord[ab].rel_position.x, 0, X.minpitch + aa / ABCJS.write.spacing.STEP, {
                                type: "text",
                                height: V
                            }))
                        } else {
                            var f = "above";
                            if (X.positioning && X.positioning.chordPosition) {
                                f = X.positioning.chordPosition
                            }
                            a = this.renderer.getTextSize(X.chord[ab].name, "gchordfont", "chord");
                            V = a.height / ABCJS.write.spacing.STEP;
                            j = a.width;
                            k.addCentered(new ABCJS.write.RelativeElement(X.chord[ab].name, P, j, undefined, {
                                type: "chord",
                                position: f,
                                height: V
                            }))
        ...
        if (X.startTriplet) {
            this.triplet = new ABCJS.write.TripletElem(X.startTriplet, v, null, true);
            if (!af) {
                this.voice.addOther(this.triplet)
            }
        }
        if (X.endTriplet && this.triplet) {
            this.triplet.setCloseAnchor(v);
            this.triplet = null;
            this.tripletmultiplier = 1
        }
        return k
...
if (!ABCJS.write) {
    ...
    ABCJS.write.BeamElem.prototype.yAtNote = function(l) {
        var k = this.beams[0];
        return b(k.startX, k.startY, k.endX, k.endY, l.x)
    };
...
ABCJS.write.CrescendoElem.prototype.draw = function(b) {
    if (this.pitch === undefined) {
        console.error("Crescendo Element y-coordinate not set.")
    }
...
if (!ABCJS.write) {
    ...
    var d = function(n, f, q, g, p, j, e) {
        ...
        function s(v, t) {
            ...
            var i = 2;
            var u = 5;
            q.addChild(new ABCJS.write.RelativeElement(v, f / 2, 0, w + i, {
                type: "decoration",
                klass: "ornament",
                thickness: 3
            }));
...
ABCJS.write.DynamicDecoration.prototype.draw = function(d, e, a) {
    if (this.pitch === undefined) {
        console.error("Dynamic Element y-coordinate not set.")
    }
    var c = 1;
    var b = 1;
...
ABCJS.write.EndingElem.prototype.draw = function(d, f, c) {
    if (this.pitch === undefined) {
        console.error("Ending Element y-coordinate not set.")
    }
...
ABCJS.write.RelativeElement = function(g, d, b, f, e) {
    e = e || {};
    this.x = 0;
    this.c = g;
    this.dx = d;
    ...
    this.type = e.type || "symbol";
    this.pitch2 = e.pitch2;
    ...
    if (e.thickness) {
        this.top += e.thickness / 2;
        this.bottom -= e.thickness / 2
    }
    if (e.stemHeight) {
        if (e.stemHeight > 0) {
            this.top += e.stemHeight
        } else {
            this.bottom += e.stemHeight
        }
    }
    var a = e.height ? e.height : 4;
    this.centerVertically = false;
    switch (this.type) {
        case "debug":
            this.chordHeightAbove = a;
            break;
        case "lyric":
            if (e.position && e.position === "below") {
                this.lyricHeightBelow = a
            } else {
                this.lyricHeightAbove = a
            }
            break;
        case "chord":
            if (e.position && e.position === "below") {
                this.chordHeightBelow = a
            } else {
                this.chordHeightAbove = a
            }
            break;
...
ABCJS.write.RelativeElement.prototype.draw = function(b, c) {
    if (this.pitch === undefined) {
        console.error(this.type + " Relative Element y-coordinate not set.")
    }
...
ABCJS.write.StaffGroupElement.prototype.layout = function(e, n, m) {
    var t = 1e-7;
    this.spacingunits = 0;
    this.minspace = 1000;
    var g = n.padding.left;
    var b = 0;
    for (var p = 0; p < this.voices.length; p++) {
        if (this.voices[p].header) {
            var l = n.getTextSize(this.voices[p].header, "voicefont", "");
            b = Math.max(b, l.width)
        }
    }
    g = g + b * 1.1;
    this.startx = g;
    ...
    for (p = 0; p < this.voices.length; p++) {
        this.voices[p].beginLayout(g)
    }
    var d = 0;
    while (!this.finished()) {
        k = null;
        for (p = 0; p < this.voices.length; p++) {
            if (!this.voices[p].layoutEnded() && (!k || this.voices[p].getDurationIndex() < k)) {
                k = this.voices[p].getDurationIndex()
            }
        }
        if (m) {
            console.log("currentduration: ", k)
        }
        var f = [];
        var c = [];
        for (p = 0; p < this.voices.length; p++) {
            var q = this.voices[p].getDurationIndex();
            if (q - k > t) {
                c.push(this.voices[p])
            } else {
                f.push(this.voices[p]);
                if (m) {
                    console.log("in: voice ", p)
                }
            }
        }
        d = 0;
        ...
        for (p = 0; p < f.length; p++) {
            if (f[p].getNextX() > g) {
                g = f[p].getNextX();
                d = f[p].getSpacingUnits();
                r = f[p].spacingduration
            }
        }
        this.spacingunits += d;
        this.minspace = Math.min(this.minspace, d);
        for (p = 0; p < f.length; p++) {
            var a = f[p].layoutOneItem(g, e);
            var h = a - g;
            if (h > 0) {
                g = a;
                for (var o = 0; o < p; o++) {
    ...
    for (p = 0; p < this.voices.length; p++) {
        if (this.voices[p].getNextX() > g) {
            g = this.voices[p].getNextX();
            d = this.voices[p].getSpacingUnits()
        }
    }
    this.spacingunits += d;
    this.w = g;
    for (p = 0; p < this.voices.length; p++) {
        this.voices[p].w = this.w
    }
};
...
if (!ABCJS.write) {
    ...
    ABCJS.write.TempoElement.prototype.draw = function(w) {
        var n = this.x;
        if (this.pitch === undefined) {
            console.error("Tempo Element y-coordinate not set.")
        }
        var l = w.calcY(this.pitch);
        var r;
        if (this.tempo.preString) {
            r = w.renderText(n, l, this.tempo.preString, "tempofont", "tempo", "start");
            var D = r.getBBox().width;
            var g = D / this.tempo.preString.length;
            n += D + g
        }
        if (this.tempo.duration) {
            var h = 0.75;
            var B = this.pitch - a + 1;
            var b = this.tempo.duration[0];
            var q = new ABCJS.write.AbsoluteElement(this.tempo, b, 1, "tempo");
            var z = Math.floor(Math.log(b) / Math.log(2));
            var t = 0;
            for (var p = Math.pow(2, z), m = p / 2; p < b; t++, p += m, m /= 2) {}
            var E = w.engraver.chartable.note[-z];
            var C = w.engraver.chartable.uflags[-z];
            var s = w.engraver.createNoteHead(q, E, {
                verticalPos: B
            }, "up", 0, 0, C, t, 0, h);
            q.addHead(s);
            var k;
            if (b < 1) {
                ...
                var o = s.dx + s.w;
                var u = -0.6;
                k = new ABCJS.write.RelativeElement(null, o, 0, f, {
                    type: "stem",
                    pitch2: e,
                    linewidth: u
                });
            ...
            q.setX(n);
            for (var A = 0; A < q.children.length; A++) {
                q.children[A].draw(w, n)
            }
            n += (q.w + 5);
            var v = "= " + this.tempo.bpm;
            r = w.renderText(n, l, v, "tempofont", "tempo", "start");
            var d = r.getBBox().width;
            var j = d / v.length;
            n += d + j
        }
        if (this.tempo.postString) {
            w.renderText(n, l, this.tempo.postString, "tempofont", "tempo", "start")
        }
    }
})();
if (!ABCJS) {
...
ABCJS.write.VoiceElement.prototype.moveDecorations = function(a) {
    var f = 1.5;
...
ABCJS.write.VoiceElement.prototype.layoutBeams = function() {
    for (var b = 0; b < this.beams.length; b++) {
        if (this.beams[b].layout) {
            this.beams[b].layout();
            this.moveDecorations(this.beams[b]);
